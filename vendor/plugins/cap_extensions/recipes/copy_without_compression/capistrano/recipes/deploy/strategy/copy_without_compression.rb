...
module Capistrano
  module Deploy
    module Strategy
      ...
      class CopyWithoutCompression < Copy
        ...
        def deploy!
          if copy_cache
            ...
            logger.debug "NOT getting (via #{copy_strategy}) revision #{revision} to #{destination}"
            # system(command)

            if copy_exclude.any?
              raise "not implemented in the short-circuited way of copying."
          ...
          logger.trace "NOT compressing #{destination} to #{filename}"
...
#            run "#{decompress(remote_filename).join(" ")}"
            system "cp -R #{repository} #{latest_release}"
#            run "rm #{remote_filename}"
          end
      end
